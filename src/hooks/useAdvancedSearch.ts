...
export const useAdvancedSearch = (options: UseAdvancedSearchOptions = {}) => {
  ...
  const performSearch = useCallback(
    async (
      ...
      try {
        const response: GeneralSearchResponse =
          await searchService.searchGeneral({
            query,
            page,
            limit: 20,
          });
        ...
        if (response.status === "success" && response.data?.results) {
          ...
          if (category === "all") {
            ...
            resultsToShow =
              response.data.results.grouped?.[category]?.data || [];
          }
          ...
          setState((prev) => ({
            ...
            results: {
              data:
                page === 1
                  ? resultsToShow
                  : [...(prev.results?.data || []), ...resultsToShow],
              ...
              loadedCount:
                page === 1
                  ...
                  : (prev.results?.loadedCount || 0) + resultsToShow.length,
            },
            loading: false,
            hasSearched: true,
          ...
          const currentLoadedCount =
            page === 1
              ? resultsToShow.length
              : (state.results?.loadedCount || 0) + resultsToShow.length;
  ...
  const loadMore = useCallback(async () => {
    ...
    const currentLoadedCount =
      state.results.loadedCount || state.results.data.length;
        ...
        switch (currentCategory) {
          ...
          case "book_lists":
            ...
            if (response.status === "success") {
              ...
              const maxToLoad = Math.min(
                state.results.totals?.book_lists || 0,
                100
              );
              const hasMoreResults =
                newLoadedCount < maxToLoad &&
  ...
  const changeCategory = useCallback(
    async (newCategory: SearchCategory) => {
      ...
      if (state.query && state.results?.fullResults) {
        ...
        if (newCategory === "all") {
          ...
          resultsToShow =
            state.results.fullResults.grouped?.[newCategory]?.data || [];
        }
        ...
        if (category === "all") {
          ...
          switch (category) {
            ...
            case "book_lists":
              ...
              if (response.status === "success") {
                newResults = (response.data.book_lists || []).map(
                  (list: any) => ({
                    ...list,
                    type: "book_list",
                  })

...
export default function GamificationPage() {
  ...
  const [inventoryShop, setInventoryShop] = useState<InventoryShop | null>(null);
  const [selectedInventoryTab, setSelectedInventoryTab] = useState<'items' | 'equipped' | 'shop' | 'stats'>('items');
  
  // Challenges state
  const [challenges, setChallenges] = useState<Challenge[]>([]);
  const [popularChallenges, setPopularChallenges] = useState<Challenge[]>([]);
  const [personalChallengeStats, setPersonalChallengeStats] = useState<PersonalChallengeStats | null>(null);
  const [selectedChallengeTab, setSelectedChallengeTab] = useState<'active' | 'popular' | 'create' | 'stats'>('active');
  ...
  const [xpToast, setXpToast] = useState<{ amount: number; reason?: string; visible: boolean }>({
    amount: 0,
    reason: '',
    visible: false
  });
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      ...
      const requiredForNextLevel = xpProgress.required || exp.xpToNextLevel || 100;
    ...
    try {
      ...
      completedMissions.forEach((mission: any) => {
        const transformedMission = {
          ...
          isAccepted: true,
          reward: {
            type: getRewardType(mission.coin_reward || 0, mission.xp_reward || 0),
            amount: mission.coin_reward && mission.coin_reward > 0 ? mission.coin_reward : mission.xp_reward || 0
          },
          expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
          narrativeContext: getNarrativeContext(mission.title, mission.description),
      ...
      try {
        ...
        let fallbackMissions = activeMissions.map((activeMission: any) => {
          ...
          return {
            ...
            category: getMissionCategory(mission.requirement?.type || 'reading'),
            difficulty: mission.difficulty,
            type: mission.frequency === 'daily' ? 'daily' : mission.frequency === 'weekly' ? 'weekly' : 'special',
            target: mission.requirement?.value || 1,
            ...
            reward: {
              type: mission.coinReward && mission.coinReward > 0 ? 'bookcoins' : 'experience',
              amount: mission.coinReward && mission.coinReward > 0 ? mission.coinReward : mission.xpReward || 0
            },
            expiresAt: activeMission.expiresAt || new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
            narrativeContext: mission.narrativeDescription || getNarrativeContext(mission.title, mission.description),
  ...
  const fetchShop = useCallback(async () => {
    try {
      ...
      try {
        const storedBonus = localStorage.getItem(STORAGE_KEYS.LOCAL_COINS_BONUS);
  ...
  const fetchEventsOverview = useCallback(async () => {
    try {
      ...
      if (multipliersData.coinMultiplier && multipliersData.coinMultiplier > 1) {
        transformedMultipliers.push({
          id: 'coin_multiplier',
      ...
      if (multipliersData.specialMultipliers && Array.isArray(multipliersData.specialMultipliers)) {
  ...
  const handleMissionAccept = useCallback(async (missionId: string) => {
    try {
      // Show loading state
  ...
  const buyItem = async (itemId: string) => {
    try {
      ...
      } else if (error instanceof Error && error.message.includes('already purchased')) {
  ...
  const joinHappyHour = async (happyHourId: string) => {
    try {
      ...
      setHappyHours(prev => prev.map(hh => 
        hh.id === happyHourId 
  ...
  const joinFlashContest = async (contestId: string) => {
    try {
      ...
      // Update participant count
      setFlashContests(prev => prev.map(contest => 
  ...
  const purchaseInventoryItem = async (itemId: string, quantity: number = 1) => {
    try {
  ...
  const updateProgress = async (challengeId: string, objectiveId: string, progress: number) => {
    try {
        ...
        challenge.id === challengeId 
          ? {
              ...
              objectives: challenge.objectives.map(obj => 
                obj.id === objectiveId 
                  ? { ...obj, current: progress, isCompleted: progress >= obj.target }
                  : obj
              )
            }
          : challenge
  ...
  const handleCreateChallenge = (type: 'reading' | 'social' | 'discovery' | 'custom') => {
  ...
  }, [fetchMissions, fetchBadges, fetchLeaderboard, fetchShop, fetchEventsOverview, fetchInventory, fetchEquippedItems, fetchChallenges, fetchPersonalChallengeStats]);
  ...
  const handleChallengeTabChange = useCallback((value: string) => {
    setSelectedChallengeTab(value as 'active' | 'popular' | 'create' | 'stats');
  ...
  const saveCompletedMission = useCallback((missionId: string, rewards: { xp: number; coins: number }) => {
    try {
      const completed = getLocalCompletedMissions();
      const existingIndex = completed.findIndex((m: any) => m.missionId === missionId);
      ...
      if (existingIndex === -1) {
        ...
        const currentXpBonus = parseInt(localStorage.getItem(STORAGE_KEYS.LOCAL_XP_BONUS) || '0');
        const currentCoinsBonus = parseInt(localStorage.getItem(STORAGE_KEYS.LOCAL_COINS_BONUS) || '0');
        
        localStorage.setItem(STORAGE_KEYS.LOCAL_XP_BONUS, (currentXpBonus + rewards.xp).toString());
        localStorage.setItem(STORAGE_KEYS.LOCAL_COINS_BONUS, (currentCoinsBonus + rewards.coins).toString());
      }
    } catch (error) {
  ...
  const getLocalBonuses = useCallback(() => {
    return {
      ...
      coins: parseInt(localStorage.getItem(STORAGE_KEYS.LOCAL_COINS_BONUS) || '0'),
      level: parseInt(localStorage.getItem(STORAGE_KEYS.LOCAL_LEVEL_BONUS) || '0')
    };
  }, []);

  // Apply local bonuses to dashboard data
  const applyLocalBonuses = useCallback((dashboardData: GamificationDashboard) => {
    ...
    return {
      ...
      weeklyProgress: {
        ...
        missionsCompleted: dashboardData.weeklyProgress.missionsCompleted + completedMissions.length
      }
    };
  }, [getLocalBonuses, getLocalCompletedMissions]);

  // Apply local completion status to missions
  const applyLocalMissionStatus = useCallback((missionsData: Mission[]) => {
    const completedMissions = getLocalCompletedMissions();
  ...
  const getMissionCategoryInfo = (category: string) => {
    const categoryData = MISSION_CATEGORIES[category as keyof typeof MISSION_CATEGORIES];
  ...
  const renderMissionCard = (mission: Mission) => {
    ...
    return (
      <Card key={mission.id} className="border border-gray-200 hover:shadow-md transition-shadow">
        <CardContent className="p-4">
          <div className="flex justify-between items-start mb-2">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs" style={{ backgroundColor: categoryInfo.color + '20' }}>
                  {categoryInfo.icon}
                </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                <Badge variant={mission.type === 'daily' ? 'default' : mission.type === 'weekly' ? 'secondary' : 'outline'}>
                  {mission.type === 'daily' ? 'Quotidien' : mission.type === 'weekly' ? 'Hebdomadaire' : 'Mensuel'}
                </Badge>
              ...
              {mission.reward && (
                <div className="flex items-center gap-3 mt-2 text-xs">
                  ...
                  <div className="flex items-center gap-1 text-amber-600">
                    ...
                    {mission.reward.amount} {mission.reward.type === 'experience' ? 'XP' : mission.reward.type === 'bookcoins' ? 'BookCoins' : 'pts'}
                  </div>
                </div>
              )}
            </div>
  ...
  const renderBadgeCard = (badge: GamificationBadge) => (
    <Card key={badge.id} className={`border ${badge.isUnlocked ? 'border-amber-300 bg-amber-50' : 'border-gray-200'}`}>
      <CardContent className="p-4 text-center">
        <div className={`w-12 h-12 mx-auto mb-2 rounded-full flex items-center justify-center ${
          badge.isUnlocked ? 'bg-amber-200' : 'bg-gray-100'
        }`}>
          <Award className={`h-6 w-6 ${badge.isUnlocked ? 'text-amber-600' : 'text-gray-400'}`} />
        </div>
  ...
  const renderLeaderboardEntry = (entry: LeaderboardEntry) => (
    <div key={entry.userId} className="flex items-center gap-3 p-3 border-b border-gray-100 last:border-b-0">
  ...
  const getShopItemIcon = (category: string) => {
    switch (category) {
      ...
      case 'cosmetic': return Sparkle;
      case 'boost': return TrendingUp;
      case 'themes': return PaintBucket;
      case 'badges': return Award;
      case 'avatars': return User;
      default: return Gift;
    }
  };
  ...
  const renderHappyHourCard = (happyHour: HappyHour) => (
    <Card key={happyHour.id} className={`border ${happyHour.isActive ? 'border-orange-300 bg-orange-50' : 'border-gray-200'}`}>
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground mb-2">{happyHour.description}</p>
            
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1 text-orange-600">
                <TrendingUp className="h-4 w-4" />
                <span className="text-xs font-bold">x{happyHour.multiplier}</span>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="text-xs text-muted-foreground">
                {new Date(happyHour.startTime).toLocaleTimeString()} - {new Date(happyHour.endTime).toLocaleTimeString()}
              </div>
  ...
  const renderFlashContestCard = (contest: FlashContest) => (
    <Card key={contest.id} className={`border ${contest.isActive ? 'border-red-300 bg-red-50' : 'border-gray-200'}`}>
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground mb-2">{contest.description}</p>
            <p className="text-xs font-medium mb-2">Objectif: {contest.objective}</p>
  ...
  const renderSpecialEventCard = (event: SpecialEvent) => (
    <Card key={event.id} className={`border ${event.isActive ? 'border-purple-300 bg-purple-50' : 'border-gray-200'}`}>
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge variant={event.isActive ? "default" : "secondary"}>
                {event.type === 'seasonal' ? 'Saisonnier' : 
                 event.type === 'anniversary' ? 'Anniversaire' : 
                 event.type === 'community' ? 'Communauté' : 'Défi'}
              </Badge>
            </div>
  ...
  const renderMultiplierCard = (multiplier: Multiplier) => (
    <Card key={multiplier.id} className={`border ${multiplier.isActive ? 'border-green-300 bg-green-50' : 'border-gray-200'}`}>
      <CardContent className="p-4">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 text-green-600">
                <span className="text-lg font-bold">x{multiplier.multiplier}</span>
              </div>
            </div>
            
            <p className="text-xs text-muted-foreground mb-1">{multiplier.description}</p>
            
            <div className="flex items-center justify-between">
              <Badge variant={multiplier.isActive ? "default" : "secondary"}>
                {multiplier.category === 'experience' ? 'Expérience' : 
                 multiplier.category === 'points' ? 'Points' : 
                 multiplier.category === 'streak' ? 'Série' : 'Tous'}
              </Badge>
  ...
  const getRarityClasses = (rarity: string) => {
    switch (rarity.toLowerCase()) {
      ...
      case 'uncommon': return 'text-green-600 border-green-600';
      case 'rare': return 'text-blue-600 border-blue-600';
      case 'epic': return 'text-purple-600 border-purple-600';
      case 'legendary': return 'text-amber-600 border-amber-600';
  ...
  const getDifficultyClasses = (difficulty: string) => {
    switch (difficulty.toLowerCase()) {
      ...
      case 'expert': return 'text-purple-600 border-purple-600';
      default: return 'text-gray-600 border-gray-600';
    }
  };
  ...
  const renderInventoryItem = (item: InventoryItem) => {
    ...
    return (
      <Card key={item.id} className={`border ${item.isEquipped ? 'border-blue-300 bg-blue-50' : 'border-gray-200'}`}>
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            <div className="w-12 h-12 bg-gradient-to-br from-purple-100 to-pink-100 rounded-lg flex items-center justify-center">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1">
                <h3 className="font-semibold text-sm">{item.name}</h3>
                <Badge variant="outline" className={getRarityClasses(item.rarity)}>
                  {item.rarity}
                </Badge>
              </div>
  ...
  const renderEquippedItem = (item: InventoryItem | undefined, label: string) => {
      ...
      <Card className="border border-gray-200">
        <CardContent className="p-4 text-center">
          <div className="w-12 h-12 mx-auto mb-2 bg-gradient-to-br from-blue-100 to-purple-100 rounded-lg flex items-center justify-center">
            <TypeIcon className={`h-6 w-6 ${item ? 'text-blue-600' : 'text-gray-400'}`} />
          </div>
  ...
  const renderChallengeCard = (challenge: Challenge) => {
    ...
    return (
      <Card key={challenge.id} className={`border ${challenge.status === 'active' ? 'border-blue-300 bg-blue-50' : 'border-gray-200'}`}>
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-1">
                  <Badge variant="outline" className={getDifficultyClasses(challenge.difficulty)}>
                    {challenge.difficulty}
                  </Badge>
                  <Badge variant={challenge.status === 'active' ? 'default' : 'secondary'}>
                    {challenge.status === 'active' ? 'Actif' : 
                     challenge.status === 'completed' ? 'Terminé' : 
                     challenge.status === 'failed' ? 'Échoué' : 'En attente'}
                  </Badge>
                </div>
              </div>
                ...
                <div className="flex gap-1">
                  ...
                  {challenge.status === 'active' && (
                    <Button 
                      size="sm" 
                      variant="outline"
                      onClick={() => updateProgress(challenge.id, challenge.objectives[0]?.id || '', challenge.objectives[0]?.current + 1 || 1)}
                      className="text-xs"
                    >
                      Progression
                    </Button>
    ...
    <div key={objective.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
      ...
      <div className="text-right">
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs text-muted-foreground">/ {objective.target}</span>
        </div>
  ...
  const getMissionCategory = (requirementType: string): 'reading' | 'exploration' | 'community' | 'creation' | 'engagement' => {
  ...
  const getMissionType = (frequency: string): 'daily' | 'weekly' | 'monthly' | 'special' => {
    switch (frequency) {
      case 'daily':
        return 'daily';
      case 'weekly':
  ...
  const getRewardType = (coinReward: number, xpReward: number): 'experience' | 'bookcoins' => {
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full mt-16">
                  ...
                  <div className="flex items-center justify-between mb-4 md:mb-6">
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <div className="text-xs md:text-sm font-medium text-gray-600 dark:text-gray-300">
                        {dashboard.experienceToNextLevel} XP pour le niveau suivant
                      </div>
                        </div>
                      </div>
                ...
                <div className={cn(
                  ...
                  {gameMode === 'zen' ? (
                    <>
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-emerald-800 dark:text-emerald-200">Mode Zen actif</h3>
                    ...
                    <>
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-orange-800 dark:text-orange-200">Mode Challenge actif</h3>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 rounded-lg px-3 py-1">
                      ...
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-200">Activité</span>
                    </div>
                        </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-gray-700 dark:text-gray-200">Missions terminées</span>
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-gray-700 dark:text-gray-200">Gels de série</span>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="text-center bg-orange-50 dark:bg-orange-900/20 rounded-lg p-4">
                        ...
                        <div className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">
                          {dashboard.currentStreak <= 1 ? 'jour consécutif' : 'jours consécutifs'}
                        </div>
                        <div className="text-xs text-gray-600 dark:text-gray-300">
                          Record : {dashboard.longestStreak} {dashboard.longestStreak <= 1 ? 'jour' : 'jours'}
                      </div>
                      </div>
                      </div>
                    </div>
                  ...
                  {gameMode === 'challenge' && dashboard.rank && (
                    <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                      <div className="flex items-center justify-between bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 rounded-lg p-3">
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-200">🏆 Rang actuel</span>
              ...
              <div className="space-y-6">
                ...
                {badges.length > 0 ? (
                  <>
                    {/* Unlocked API Badges */}
                    <div className="space-y-4">
                      <h3 className="text-lg font-semibold text-green-600">🏆 Badges obtenus</h3>
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {badges
                      ...
                      {badges.filter(b => b.isUnlocked).length === 0 && (
                        <div className="text-center py-8 text-muted-foreground">
                          ...
                          <p className="text-sm">Continuez à lire pour obtenir vos premiers badges !</p>
                        </div>
                      )}
                    </div>
                    ...
                    {/* Locked API Badges */}
                    <div className="space-y-4">
                      <h3 className="text-lg font-semibold text-gray-600">🎯 Badges à débloquer</h3>
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {badges
                          .filter(badge => !badge.isUnlocked)
                          .slice(0, 6)
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-center py-12">
                      <Trophy className="w-16 h-16 text-muted-foreground/50 mx-auto mb-4" />
                      <h3 className="text-lg font-semibold mb-2">Classement en construction</h3>
                      <p className="text-muted-foreground">
                        Le classement se remplira au fur et à mesure que les utilisateurs participent aux défis.
                      </p>
                      <p className="text-sm text-muted-foreground mt-2">
                        Continuez à lire et à terminer des missions pour apparaître dans le classement !
                      </p>
                  </div>
                )}
                    </CardContent>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold text-amber-800">Mes BookCoins</h3>
                          <p className="text-sm text-amber-600">Votre monnaie virtuelle</p>
                </div>
            </div>
                      <div className="text-right">
                        <div className="text-2xl font-bold text-amber-700">{userCoins}</div>
                ...
                <div>
                  ...
                  {shopItems.length > 0 ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <h3 className="text-lg font-semibold mb-2">Boutique temporairement vide</h3>
                      <p className="text-muted-foreground">
          ...
          <TabsContent value="inventory">
            <div className="space-y-6">
              ...
              <Tabs value={selectedInventoryTab} onValueChange={handleInventoryTabChange}>
                ...
                <TabsContent value="shop">
                  {loadingStates.inventoryShop ? (
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                      {(inventoryShop?.items || []).map(renderInventoryShopItem)}
                        </div>
                )}
              </TabsContent>
                ...
                <TabsContent value="stats">
                  {loadingStates.inventoryStats ? (
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <CardContent className="p-6">
                          <div className="text-center">
                            ...
                            <p className="text-sm text-muted-foreground">Total des objets</p>
                          </div>
                            </CardContent>
                          </Card>
                          <Card>
                        <CardContent className="p-6">
                          <div className="text-center">
                              <p className="text-2xl font-bold">{inventoryStats.totalValue}</p>
                        ...
                        <CardContent className="p-6">
                          <div className="text-center">
                            <p className="text-2xl font-bold">{Object.keys(inventoryStats.itemsByType).length}</p>
                            <p className="text-sm text-muted-foreground">Types d'objets</p>
                            </div>
                          </CardContent>
                        </Card>
                        <Card>
                        <CardContent className="p-6">
                          <div className="text-center">
                            <p className="text-2xl font-bold">{Object.keys(inventoryStats.itemsByRarity).length}</p>
                            <p className="text-sm text-muted-foreground">Raretés</p>
                                </div>
                        </CardContent>
                      </Card>
                            </div>
                    ...
                    <Card>
                      <CardContent className="p-6">
                        <p className="text-center text-muted-foreground">Aucune statistique disponible</p>
                          </CardContent>
                        </Card>
                )}
              </TabsContent>
              ...
              onRewardReceived={(reward) => {
                ...
                if (reward.type === 'bookcoins' && reward.amount) {
                  setDashboard(prev => prev ? {
                    ...
                    currency: {
                      ...prev.currency,
                      bookcoins: prev.currency.bookcoins + (reward.amount || 0)
                    }
                  } : null);
                ...
                if (reward.rarity === 'legendary' || reward.rarity === 'epic') {
                  setCelebrationData({
                    ...
                    bookcoins: reward.type === 'bookcoins' ? (reward.amount || 0) : 0
                  });
                  setShowMissionCelebration(true);
                }
              }}
              ...
              <div className="space-y-6">
                ...
                <XPBonusSystem
                  ...
                  onClaimDailyChest={() => {
                    ...
                    toast.error("Fonctionnalité non disponible - API manquante");
                  }}
                ...
                <div className="border-t pt-6">
                  <h2 className="text-2xl font-bold mb-6">Événements Spéciaux</h2>
                ...
                {(specialEvents || []).length > 0 && (
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold">Événements Spéciaux</h3>
                    <div className="grid gap-4 md:grid-cols-2">
                  ...
                  {(multipliers || []).length > 0 && (
                    <div className="space-y-4">
                      <h3 className="text-lg font-semibold">Multiplicateurs Actifs</h3>
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <TabsContent value="challenges">
            {loadingStates.challenges ? (
              ...
              <div className="space-y-6">
                ...
                <Tabs value={selectedChallengeTab} onValueChange={handleChallengeTabChange}>
                  <TabsList className="grid w-full grid-cols-4">
                    ...
                    <div className="space-y-4">
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="grid gap-4 md:grid-cols-2">
                            <Button onClick={() => handleCreateChallenge('reading')}>
                              <BookOpen className="w-4 h-4 mr-2" />
                              Défi de Lecture
                            </Button>
                            <Button onClick={() => handleCreateChallenge('social')}>
                              <Users className="w-4 h-4 mr-2" />
                              Défi Social
                            </Button>
                            <Button onClick={() => handleCreateChallenge('discovery')}>
                              <Sparkles className="w-4 h-4 mr-2" />
                              Défi Découverte
                            </Button>
                            <Button onClick={() => handleCreateChallenge('custom')}>
                  ...
                  <TabsContent value="stats">
                    {personalChallengeStats ? (
                      ...
                      <Card>
                          <CardContent className="p-6">
                            <div className="text-center">
                              <p className="text-2xl font-bold">{personalChallengeStats.totalChallenges}</p>
                              <p className="text-sm text-muted-foreground">Total des défis</p>
                            </div>
                            </CardContent>
                          </Card>
                          <Card>
                          <CardContent className="p-6">
                            <div className="text-center">
                              <p className="text-2xl font-bold">{personalChallengeStats.completedChallenges}</p>
                          ...
                          <CardContent className="p-6">
                            <div className="text-center">
                              <p className="text-2xl font-bold">{Math.round(personalChallengeStats.successRate)}%</p>
                        ...
                        <Card>
                          <CardContent className="p-6">
                            <div className="text-center">
                              <p className="text-2xl font-bold">{personalChallengeStats.longestStreak}</p>

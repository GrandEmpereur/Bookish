...
export default function EditBookList({ params }: PageProps) {
  ...
  const onSubmit = async (data: UpdateBookListInput) => {
    try {
      ...
      if (dirtyFields.coverImage !== undefined)
        formData.coverImage = data.coverImage;

      await bookListService.updateBookList(id, formData);

      toast.success("Liste de lecture mise à jour avec succès");

      router.push(`/library/${id}`);
    } catch (error: any) {
      toast.error(
        error.message || "Impossible de mettre à jour la liste de lecture"
      );
    } finally {
      setIsLoading(false);
    }

...
class LikeService {
    ...
    async togglePostLike(postId: string): Promise<ToggleLikeResponse> {
        return await apiRequest<ToggleLikeResponse>('POST', '/like/post', { data: { postId } });
    }

    async toggleCommentLike(commentId: string): Promise<ToggleLikeResponse> {
        return await apiRequest<ToggleLikeResponse>('POST', '/like/comment', { data: { commentId } });
    }

    async checkIsLiked(type: 'post' | 'comment', id: string): Promise<ApiResponse<{ is_liked: boolean }>> {
        return await apiRequest<ApiResponse<{ is_liked: boolean }>>('GET', `/${type}s/${id}/like/check`);
    }

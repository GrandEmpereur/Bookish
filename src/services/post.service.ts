...
class PostService {
    ...
    async updatePost(id: string, data: UpdatePostRequest): Promise<ApiResponse<UpdatePostResponse>> {
        ...
        return await apiRequest<ApiResponse<UpdatePostResponse>>('PATCH', `/posts/${id}`, { data: formData });
    }
    ...
    async toggleLike(id: string): Promise<ApiResponse<ToggleLikeResponse>> {
        return await apiRequest<ApiResponse<ToggleLikeResponse>>('POST', `/posts/${id}/like`);
    }

    async toggleFavorite(id: string): Promise<ApiResponse<ToggleFavoriteResponse>> {
        return await apiRequest<ApiResponse<ToggleFavoriteResponse>>('POST', `/posts/${id}/favorite`);
    }

    async reportPost(id: string, reason: string): Promise<ApiResponse<null>> {
        return await apiRequest<ApiResponse<null>>('POST', `/posts/${id}/report`, { data: { reason } });
    }
}

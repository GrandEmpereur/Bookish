...
class ClubService {
    async getClubs(options?: {
        ...
        return await apiRequest<ApiResponse<GetClubsResponse>>('GET', '/clubs', { params });
    }

    async getClub(id: string): Promise<ApiResponse<GetClubResponse>> {
        return await apiRequest<ApiResponse<GetClubResponse>>('GET', `/clubs/${id}`);
    }
    ...
    async createClub(data: CreateClubRequest): Promise<ApiResponse<CreateClubResponse>> {
        ...
        return await apiRequest<ApiResponse<CreateClubResponse>>('POST', '/clubs', { data: formData });
    }
    ...
    async updateClub(id: string, data: UpdateClubRequest): Promise<ApiResponse<UpdateClubResponse>> {
        ...
        return await apiRequest<ApiResponse<UpdateClubResponse>>('PATCH', `/clubs/${id}`, { data: formData });
    }
    ...
    async getMessages(clubId: string, page = 1): Promise<ApiResponse<GetClubMessagesResponse>> {
        return await apiRequest<ApiResponse<GetClubMessagesResponse>>('GET', `/clubs/${clubId}/messages`, { params: { page } });
    }
    ...
    async sendMessage(clubId: string, data: SendMessageRequest): Promise<ApiResponse<SendMessageResponse>> {
        ...
        return await apiRequest<ApiResponse<SendMessageResponse>>('POST', `/clubs/${clubId}/messages`, { data: formData });
    }

    async joinClub(id: string, data: JoinClubRequest): Promise<ApiResponse<JoinClubResponse>> {
        return await apiRequest<ApiResponse<JoinClubResponse>>('POST', `/clubs/${id}/join`, { data });
    }
    ...
    async generateInviteLink(clubId: string): Promise<ApiResponse<GenerateInviteLinkResponse>> {
        return await apiRequest<ApiResponse<GenerateInviteLinkResponse>>('POST', `/clubs/${clubId}/invite`);
    }

    async banMember(clubId: string, userId: string): Promise<ApiResponse<null>> {
        return await apiRequest<ApiResponse<null>>('POST', `/clubs/${clubId}/ban/${userId}`);
    }

    async reportMessage(messageId: string): Promise<ApiResponse<null>> {
        return await apiRequest<ApiResponse<null>>('POST', `/clubs/messages/${messageId}/report`);
    }
}

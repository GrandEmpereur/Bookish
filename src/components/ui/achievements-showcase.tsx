...
interface RecentActivity {
  id: string;
  type: 'book_completed' | 'badge_earned' | 'review_written' | 'friend_added' | 'level_up';
  title: string;
  description: string;
  date: string;
  metadata?: {
...
const READING_MILESTONES = [
  { books: 1, title: "Premier Livre", description: "Votre première lecture complète", unlocked: true },
  { books: 5, title: "Lecteur Débutant", description: "5 livres terminés", unlocked: true },
  { books: 10, title: "Lecteur Régulier", description: "10 livres terminés", unlocked: true },
  { books: 25, title: "Bibliophile", description: "25 livres terminés", unlocked: true },
  { books: 50, title: "Grand Lecteur", description: "50 livres terminés", unlocked: false },
  { books: 100, title: "Maître Lecteur", description: "100 livres terminés", unlocked: false },
  { books: 250, title: "Sage des Livres", description: "250 livres terminés", unlocked: false },
  { books: 500, title: "Légende Vivante", description: "500 livres terminés", unlocked: false }
];
...
export function AchievementsShowcase({ 
  ...
  const getActivityIcon = (type: RecentActivity['type']) => {
    switch (type) {
      ...
      case 'review_written': return PenTool;
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Votre parcours de lecture et vos accomplissements</p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
            ...
            <div className="text-center">
              <div className="text-lg font-bold">{stats.averageRating.toFixed(1)}/5</div>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-xl font-bold">{stats.totalPages.toLocaleString()}</div>
                  <div className="text-sm text-muted-foreground">Pages lues</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold">{stats.genresExplored}</div>
                  <div className="text-sm text-muted-foreground">Genres explorés</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold">{stats.reviewsWritten}</div>
                  <div className="text-sm text-muted-foreground">Critiques écrites</div>
                </div>
                <div className="text-center">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <div className="text-xl font-bold">{stats.commentsWritten}</div>
            ...
            {userBadges.map(badge => (
              <div key={badge.id} onClick={() => setSelectedBadge(badge)}>
                <EnhancedBadgeCard 
                  badge={badge} 
                  isCompact={true}
                  onShare={onShareAchievement ? () => onShareAchievement(badge.id) : undefined}
                />
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {READING_MILESTONES.map((milestone, index) => (
              <Card key={index} className={cn(
                ...
                milestone.unlocked ? "border-green-500 bg-green-50" : "border-gray-200 opacity-60"
              )}>
                <CardContent className="p-4">
                  <div className="flex items-center gap-4">
                    <div className={cn(
                      "w-12 h-12 rounded-full flex items-center justify-center",
                      milestone.unlocked ? "bg-green-500 text-white" : "bg-gray-300 text-gray-600"
                    )}>
                      {milestone.unlocked ? (
        ...
        <DialogContent className="max-w-2xl">
          ...
          {selectedBadge && (
            <EnhancedBadgeCard 
              ...
              onShare={onShareAchievement ? () => onShareAchievement(selectedBadge.id) : undefined}
            />
          )}
        </DialogContent>
      </Dialog>

...
export function CosmeticShop({ userBookcoins, onPurchase, onEquip, className }: CosmeticShopProps) {
  ...
  const getTypeIcon = (type: CosmeticItem['type']) => {
    switch (type) {
      ...
      case 'background': return Mountain;
      default: return Gift;
    }
  };
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Personnalisez votre expérience Bookish</p>
        </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-gray-100 rounded-lg">
                  {React.createElement(getTypeIcon(selectedItem.type), { className: "w-8 h-8 text-gray-600" })}
                </div>
                <div>
                  <h3 className="font-semibold">{selectedItem.name}</h3>
                  <p className="text-sm text-muted-foreground">{selectedItem.description}</p>
                  <Badge variant="outline" className={cn("mt-1", getRarityColor(selectedItem.rarity))}>
                    {selectedItem.rarity}
                  </Badge>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-yellow-50 rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  <Coins className="w-5 h-5 text-yellow-600" />
                  <span className="font-bold text-lg">{selectedItem.price} BookCoins</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className={cn(
                  "font-medium",
                  userBookcoins - selectedItem.price >= 0 ? "text-green-600" : "text-red-600"
                )}>

...
const generatePosts = (clubId: string, authorName: string, count: number) => {
  return Array.from({ length: count }, (_, i) => ({
    ...
    image:
      Math.random() > 0.5
        ? `https://picsum.photos/seed/post${clubId}${i}/600/400`
        : null,
    createdAt: ["1h", "2h", "3h", "6h", "12h", "1j", "2j", "3j"][
...
const generateChatMessages = (
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      ).toISOString(), // Messages des 7 derniers jours
...
export const MOCK_CLUBS = [
  {
    ...
    name: "Leo club",
    description:
      "Un club dédié aux passionnés de lecture et de découvertes littéraires",

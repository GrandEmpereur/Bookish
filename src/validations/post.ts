...
export const createPostSchema = z.object({
    title: z.string()
        ...
        .trim(),
    subject: z.enum(['book_review', 'book_recommendation', 'reading_progress',
        'book_discussion', 'author_spotlight', 'reading_challenge',
        'book_quote', 'book_collection', 'reading_list', 'literary_event'] as const, {
    ...
    content: z.string()
        ...
        .trim(),
    media: z.array(z.custom<File>((file) => file instanceof File, "Le fichier n'est pas valide")
        .refine((file) => file.size <= MAX_FILE_SIZE, "La taille du fichier ne doit pas dépasser 10MB")
        .refine((file) => ACCEPTED_IMAGE_TYPES.includes(file.type), "Type de fichier non supporté"))
        .optional()
});

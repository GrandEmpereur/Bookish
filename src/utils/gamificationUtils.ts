...
export function getRandomNarrativeMessage(
  category: keyof typeof NARRATIVE_MESSAGES
): string {
...
export const GAME_MODES = {
  zen: {
    name: "Mode Zen",
    description:
      "Progressez à votre rythme dans la sérénité de la Grande Bibliothèque",
    icon: "🧘",
    features: [
  ...
  reading: [
    {
      ...
      narrativeContext:
        "Les anciens grimoires révèlent leurs secrets à ceux qui persévèrent...",
    },
    {
      id: "read_pages",
      ...
      narrativeContext:
        "Chaque page tournée vous rapproche de la sagesse universelle.",
    },
    {
      id: "reading_streak",
      ...
      narrativeContext:
        "La flamme de la connaissance ne doit jamais s'éteindre.",
    },
  ],
  exploration: [
    {
      ...
      narrativeContext:
        "Les territoires inconnus recèlent des trésors insoupçonnés.",
    },
    {
      id: "author_discovery",
    ...
    },
    {
      ...
      narrativeContext:
        "La diversité des lectures enrichit l'esprit et ouvre les horizons.",
    },
  ],
  community: [
    {
      ...
      narrativeContext:
        "Votre sagesse guidera d'autres lecteurs dans leur quête.",
    },
    {
      ...
      narrativeContext:
        "Les maîtres transmettent leur savoir aux nouveaux apprentis.",
    },
    {
      id: "join_discussion",
  ...
  creation: [
    {
      ...
      narrativeContext:
        "Les analyses approfondies éclairent les subtilités des œuvres.",
    },
    {
      id: "create_list",
      ...
      narrativeContext:
        "Les collections thématiques sont des guides précieux pour les lecteurs.",
    },
    {
      id: "photo_challenge",
      ...
      narrativeContext:
        "Les images capturent l'essence de nos moments de lecture.",
    },
  ],
  engagement: [
    {
      ...
      rewards: { experience: 100, bookcoins: 50 },
      narrativeContext:
        "La régularité est la clé de la progression dans la Grande Bibliothèque.",
    },
    {
      ...
      narrativeContext:
        "Chaque mission accomplie vous rapproche de votre destinée.",
    },
    {
      id: "weekend_reading",
      ...
      rewards: { experience: 150, bookcoins: 75 },
      narrativeContext:
        "Les week-ends offrent des moments privilégiés pour la contemplation.",
    },
  ],
} as const;
    ...
    objectives: [
      {
        ...
        target: 4,
        title: "Explorer 4 genres différents",
      },
...
export const MONTHLY_CHALLENGE_TEMPLATES = [
  {
    ...
    narrativeStory:
      "Ce mois-ci, entreprenez une odyssée littéraire qui vous mènera à travers des mondes fantastiques, des histoires captivantes et des sagesses ancestrales. Chaque livre terminé vous rapprochera de la maîtrise ultime.",
    objectives: [
      { type: "books_read", target: 8, title: "Terminer 8 grimoires sacrés" },
      {
        type: "pages_read",
        target: 2000,
        title: "Parcourir 2000 pages de sagesse",
      },
      {
        type: "genres_explored",
        target: 5,
        title: "Explorer 5 royaumes différents",
      },
    ],
    rewards: [
  ...
  },
  {
    ...
    narrativeStory:
      "Les classiques de la littérature sont les piliers de la Grande Bibliothèque. Ce mois-ci, plongez dans ces œuvres intemporelles qui ont façonné notre compréhension du monde.",
    objectives: [
...
export const generateDailyMissions = (userProfile: {
  level: number;
  preferences: string[];
  recentGenres: string[];
...
function generateAdaptiveReadingMission(userProfile: {
  level: number;
  preferences: string[];
  recentGenres: string[];
}) {
  ...
  if (userProfile.level < 5) {
    ...
    return {
      ...
      createdAt: new Date().toISOString(),
      narrativeContext:
        "Les secrets des grimoires se révèlent à votre esprit attentif.",
    };
  } else {
    ...
    return {
      ...
      expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
      narrativeContext:
        "La concentration profonde déverrouille les mystères les plus profonds.",
    };
  }
}

function generateExplorationMission(userProfile: {
  level: number;
  preferences: string[];
  recentGenres: string[];
}) {
  ...
  const unexploredGenres = allGenres.filter(
    (genre) => !userProfile.recentGenres.includes(genre)
  );
  ...
  const targetGenre =
    unexploredGenres[Math.floor(Math.random() * unexploredGenres.length)];
...
function generateCommunityMission(userProfile: {
  level: number;
  preferences: string[];
  recentGenres: string[];
}) {
  ...
  const action =
    communityActions[Math.floor(Math.random() * communityActions.length)];
...
function generateCreationMission(userProfile: {
  level: number;
  preferences: string[];
  recentGenres: string[];
}) {
  ...
  const action =
    creationActions[Math.floor(Math.random() * creationActions.length)];
...
function generateEngagementMission(userProfile: {
  level: number;
  preferences: string[];
  recentGenres: string[];
}) {
  ...
  const action =
    engagementActions[Math.floor(Math.random() * engagementActions.length)];
...
function generateCompetitiveMission(userProfile: {
  level: number;
  preferences: string[];
  recentGenres: string[];
}) {
  const competitiveActions = [
    ...
    },
    {
      ...
      target: 1,
      narrativeContext:
        "La vitesse sans la compréhension n'est rien, mais les deux ensemble font les maîtres.",
    },
  ];

  const action =
    competitiveActions[Math.floor(Math.random() * competitiveActions.length)];
  const difficulty = getDifficultyForLevel(userProfile.level, "engagement");
  ...
  return {
    ...
    reward: {
      ...
      experience: Math.floor(
        calculateReward(userProfile.level, "engagement").experience * 1.3
      ), // 30% bonus pour défis compétitifs
      bookcoins: Math.floor(
        calculateReward(userProfile.level, "engagement").bookcoins * 1.2
      ), // 20% bonus
    },
    expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
    narrativeContext: action.narrativeContext,
...
): { experience: number; bookcoins: number; badge?: string } {
  const baseXP = 50;
  const baseCoins = 25;
...
export const generateWeeklyChallenge = () => {
  ...
  return {
    ...
    objective: {
      ...
      type: template.objectives[0].type as
        | "books_read"
        | "pages_read"
        | "time_spent"
        | "streak_days"
